...
export default ({ act, store, action, handle, cookies, route }) => ({
  MANAGEMENT_LIST_CONSUMERS_UNSUB: async () => act('UNSUB', { id: 'managementConsumers' }),

  MANAGEMENT_LIST_CONSUMERS_SUB: async ({ offset = 0, limit = 15 }, keywords, status = 2) => {
    await act('APP_SOCKET_CHECK')
    store.get('managementConsumers') && await act('MANAGEMENT_LIST_CONSUMERS_UNSUB')
    ...
    return act('SUB', {
      ...
      action: data => act('MANAGEMENT_LIST_CONSUMERS_SET', data, condition)
    })
  },

  MANAGEMENT_LIST_CONSUMERS_SET: async (data, condition) => {
  ...
  MANAGEMENT_LIST_CONSUMERS_ASSIGN: async (consumerIds, user) => {
    handle.loading(true)
    const result = await act('GQL', {
      ...
      variables: { objects: consumerIds.map(id => ({ consumerId: id, userId: user.value, type: 'consumer', finished: '', assignerId: store.get('user').id })) }
    }).catch(error => console.log(error))
  },

  MANAGEMENT_LIST_CONSUMERS_ASSIGN_SELF: async ({ id }) => {
    handle.loading(true)
    ...
    if(['associate'].includes(user.role) && !!assignments.length) {
