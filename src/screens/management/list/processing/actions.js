...
function setAddress({ city, district, commune, address }) {
  return (city && [city, district, commune].filter(v => v && v.length).join(', ')) || address || 'N/A'
}
...
export default ({ act, store, action, handle, cookies, route }) => ({
  PROCESSING_UNSUB: async () => act('UNSUB', { id: 'processing' }),

  PROCESSING_SUB: async ({ offset = 0, limit = 15 }, keywords, status = 2) => {
    await act('APP_SOCKET_CHECK')
    store.get('processing') && await act('PROCESSING_UNSUB')
    ...
    const condition = `where: { _and: [ {userId: { _is_null: false }}, {finishedAt: { _is_null: true }}, ${search}]}`
    ...
    return act('SUB', {
      ...
      action: data => act('PROCESSING_SET', data, condition)
    })
  },

  PROCESSING_SET: async (data, condition) => {
    const processingCount = await act('GQL', { query: `{ Assignments_aggregate(${condition}) { aggregate { count } } }` })
      .then(({ Assignments_aggregate: { aggregate: { count }}}) => count)

    const processing = data && data.map(item => ({
      ...item,
      ...
      finishedAt: setDate(item.finishedAt),
    ...
    return store.set({ processing, processingCount, loading: null })
  },

  PROCESSING_REASSIGN: async (assignmentIds, user) => {
    handle.loading(true)
    await act('GQL', { query: `mutation {
      update_Assignments(where: {id: {_in: ${JSON.stringify(assignmentIds)}}}, _set: {
        userId: "${user.value}",
        assignerId: "${store.get('user').id}",
  ...
  PROCESSING_ASSIGNMENTS_DELETE: async assignmentIds => handle.confirm(() => act('GQL', { query: `mutation {
    delete_Assignments(where: {id: {_in: ${JSON.stringify(assignmentIds)}}}) { affected_rows }
  }`}).catch(handle.info))
})
