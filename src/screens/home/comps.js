...
export default ( props ) => {
	const { act, store, cookies } = useActStore(actions)
	...
	useEffect(() => {
		!stats && socket && cookies.get('token') && act('STATS_FETCH')
	}, [socket])
	...
	return (
		<Layout title="Traffic Light">
				<div className="w:100pc dp:flx jc:c flxw:wrap fd:col md-fd:row md-m-rl:10npx">
					...
					<div className={classNameCardWrap(index == 0)}>
						<Card 
							key={item.label} 
							link={item.label} 
							label={item.label} 
...
const classNameCardWrap = ( index ) =>
	fucss({
		'w:100pc md-w:50pc md-p-rl:20px dp:flx ai:c jc:c bd-t:1px-sd-e8e8e8 md-bd-t:0 md-bd-l:1px-sd-e8e8e8': true,
		'bd-t:0-! bd-l:0-!': index
	});